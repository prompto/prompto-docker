...
RUN mvn dependency:get -DincludeScope=compile -Dartifact=org.prompto:Server:$(cat platform-version.txt)
RUN mvn dependency:get -DincludeScope=compile -Dartifact=org.prompto:AwsClient:$(cat platform-version.txt)
...
USER prompto
RUN mvn dependency:copy-dependencies -DincludeScope=compile -DoutputDirectory=/AwsClient -f $(cat AwsClient-prefix.txt).pom
